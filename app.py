...
with col_h3:
    st.markdown("<p class='status-online'>● ACTIVE THREAT | ● AI ONLINE | 22:13:36</p>", unsafe_allow_html=True)
...
for i, col in enumerate([m1, m2, m3, m4]):
    with col:
        ...
        <div class="css-card">
            ...
            <div style="color:{metrics[i][2]}; font-size: 28px; font-weight: bold;">{metrics[i][1]}</div>
        </div>
        """, unsafe_allow_html=True)
...
# MIDDLE COLUMN: Visualizations & Analysis
with col_mid:
    # 1. The Vector/Attack Map (Using ECharts for that techy look)
    options = {
...
with col_right:
    ...
    for title, sev in feeds:
        ...
        <div style="font-size: 12px; margin-bottom: 10px; border-left: 3px solid #ff3366; padding-left: 5px;">
            <b>{title}</b><br><span style="color: grey;">Severity: {sev}</span>
        </div>
        """, unsafe_allow_html=True)
